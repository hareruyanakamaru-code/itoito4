...
export default function HeroCarousel() {
  ...
  return (
    <section className="relative h-[540px] sm:h-[620px] overflow-hidden bg-stone-900">
      ...
      <button
        onClick={prev}
        className="absolute left-3 sm:left-6 top-1/2 -translate-y-1/2 z-30 w-11 h-11 rounded-full bg-black/25 hover:bg-black/45 backdrop-blur-sm flex items-center justify-center text-white text-3xl transition-all font-thin"
        aria-label="前のスライド"
      >
        ‹
      </button>
      <button
        onClick={next}
        className="absolute right-3 sm:right-6 top-1/2 -translate-y-1/2 z-30 w-11 h-11 rounded-full bg-black/25 hover:bg-black/45 backdrop-blur-sm flex items-center justify-center text-white text-3xl transition-all font-thin"
        aria-label="次のスライド"
      >
        ›
      </button>
